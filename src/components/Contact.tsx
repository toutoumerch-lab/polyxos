        ...
        <div className="grid lg:grid-cols-5 gap-12">
          ...
          <div className={`lg:col-span-2 transition-all duration-700 delay-200 ${inView ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
              ...
              {[
                ...
                return (
                  <a
                    key={item.label}
                    href={item.href}
                    className="flex items-start gap-4 p-5 glass rounded-2xl border border-app-border/5 hover:border-blue-500/30 transition-all duration-300 group"
                  >
                    ...
                    <div>
                      <p className="text-fg-subtle text-xs mb-1">{item.label}</p>
            ...
            <div className="glass rounded-2xl p-6 border border-app-border/5">
              <h4 className="font-display font-bold text-fg mb-4">What happens next?</h4>
              <div className="space-y-3">
                {[
                  ...
                  <div key={i} className="flex items-start gap-3">
                    ...
                    <p className="text-fg-muted text-sm">{step}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className={`lg:col-span-3 transition-all duration-700 delay-300 ${inView ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
            <div className="glass-strong rounded-3xl p-6 sm:p-8 lg:p-10 border border-app-border/10 relative overflow-hidden">
              {/* Accent */}
              ...
              {submitted ? (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <h3 className="font-display text-2xl font-bold text-fg mb-3">Message Received!</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <h3 className="font-display text-xl font-bold text-fg mb-8">Tell Us About Your Project</h3>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label htmlFor="contact-name" className="block text-xs font-semibold text-fg-muted mb-2 uppercase tracking-wide">
                        Your Name *
                      </label>
                      <input
                        id="contact-name"
                        ...
                        className="w-full px-4 py-3 rounded-xl text-sm text-fg placeholder:text-fg-subtle outline-none transition-all duration-300 focus:ring-2 focus:ring-blue-500/30"
                        style={{ background: 'rgb(var(--c-surface) / 0.05)', border: '1px solid rgb(var(--c-border) / 0.08)' }}
                      />
                    </div>
                    <div>
                      <label htmlFor="contact-email" className="block text-xs font-semibold text-fg-muted mb-2 uppercase tracking-wide">
                        Email Address *
                      </label>
                      <input
                        id="contact-email"
                  ...
                  <div>
                    <label htmlFor="contact-service" className="block text-xs font-semibold text-fg-muted mb-2 uppercase tracking-wide">
                      Service Needed
                    </label>
                    <select
                      id="contact-service"
                      ...
                      className="w-full px-4 py-3 rounded-xl text-sm text-fg outline-none transition-all duration-300 focus:ring-2 focus:ring-blue-500/30 appearance-none cursor-pointer"
                      style={{ background: 'rgb(var(--c-surface) / 0.05)', border: '1px solid rgb(var(--c-border) / 0.08)' }}
                    >
                      <option value="" style={{ background: 'rgb(var(--c-base))', color: 'rgb(var(--c-fg))' }}>Select a service...</option>
                      {services.map((s) => (
                        <option key={s} value={s} style={{ background: 'rgb(var(--c-base))', color: 'rgb(var(--c-fg))' }}>{s}</option>
                      ))}
                    </select>
                  </div>

                  <div>
                    <label htmlFor="contact-message" className="block text-xs font-semibold text-fg-muted mb-2 uppercase tracking-wide">
                      Project Details *
                    </label>
                    <textarea
                      id="contact-message"
                      ...
                      className="w-full px-4 py-3 rounded-xl text-sm text-fg placeholder:text-fg-subtle outline-none transition-all duration-300 focus:ring-2 focus:ring-blue-500/30 resize-none"
                      style={{ background: 'rgb(var(--c-surface) / 0.05)', border: '1px solid rgb(var(--c-border) / 0.08)' }}
                    />
                  </div>
